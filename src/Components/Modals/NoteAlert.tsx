...
export default function NoteAlert({ visible, onClose }: NoteAlertProps) {
  return (
    <Modal
      visible={visible}
      transparent={true}
        ...
        <View className="bg-white p-6 w-96 h-[40%]">
          <View className="border border-black h-[100%]">
            ...
            <View className="flex-1 justify-between top-10 items-center">
              <Text className="text-3xl font-satoshibold">PLEASE NOTE</Text>
            </View>
            
            {/* Content */}
            <Text className="text-2xl ml-10 mr-10 font-satoshi text-black mb-14 text-center">
              All bookings go through approval. We'll notify you once confirmed.
            </Text>
            
            {/* Action Button */}
            <TouchableOpacity 
              onPress={onClose}
              className="bg-black rounded-full w-40 py-4 left-20 bottom-10"
            >
